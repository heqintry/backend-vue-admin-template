...
export default {
  ...
  methods: {
    async getList() {
      ...
      const { data } = await permission.index();
    ...
    createData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          permission.create(this.temp).then(response => {
    ...
    updateData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          permission.update(this.tempid, this.temp).then(response => {
            if (response.code == 20000) {
              this.dialogFormVisible = false;
              this.getList();
            } else {
    ...
    handleDelete(row, index) {
      this.$confirm("Confirm Delete?", "Notice", {
        ...
        permission.delete(row.name).then(response => {
          if (response.code == 20000) {
            this.getList();
          } else {
            this.$notify({
